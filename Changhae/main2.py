...
def main(args):
    ...
    for row in tqdm(df):
        ...
        outputs_list.append(pred_answer + f'\ngold label : {gold_label}')

        # 정답 비율
        matched_num, sub_matched_result = evaluate_answer(pred_label, gold_label, matched_num, sub_matched_result)

    # 결과 저장
    with open(f"/workspace/project/AGI_finalproject/sangbeom/results2/results3_candidate_num{candidate_num}_template_idx{template_idx}.txt", "w", encoding="utf-8") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--candidate_num', help="candidate_num", type=int, default=4)
    parser.add_argument('--template_idx', help="template_idx", type=int, default=0)
